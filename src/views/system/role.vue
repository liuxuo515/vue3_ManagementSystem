...
import { getRole, saveorupdaterole, delrole } from "@/api";
...
const getData = async () => {
  const res = await getRole(query.name, "", page.index, page.size);
...
const updateData = async (form) => {
  ...
  //修改或者保存;
  const res = await saveorupdaterole(form);
  if (res.code == 200) {
...
const handlePermission = (row: Role) => {
  ...
  console.log("permissOptions", permissOptions);
};
